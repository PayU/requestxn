...
requester.list = decorateMethod('list');
npm outrequester.delete = decorateMethod('delete');
...
function defaults(defaultOptions) {
    ...
    requester.list = decorateMethod('list', defaultOptions);
...
function decorateMethod(method, defaultsOptions) {
    return (uri, options) => {
        ...
        // verbs that exists and doesn't supported in request-promise-native are send in option
        let rpFunc;
        if (customVerbs.includes($options.method.toUpperCase())) {
            $options.method = $options.method.toUpperCase();
            rpFunc = rp;
        } else {
            $options.method = $options.method.toLowerCase();
            rpFunc = rp[$options.method];
        }

        return retry(() => {
            attempts++;
            return rpFunc($options)
...
function buildRequestOptions(options) {
    return Object.assign({}, options, {
        originalSimpleValue: options.simple,
        originalResolveWithFullResponse: options.resolveWithFullResponse,
        simple: false,
...
function handleError(options, error, attempts) {
    ...
    const errorCode = error.error && error.error.code;
    const excludeError = Array.isArray(excludeErrorsFromRetry) && excludeErrorsFromRetry.includes(errorCode);
